...
{
	int retorno =-1;
	int index = Estadia_buscarEspacioLibre(list, len);
...
int EstadiaDiaria_PedirIdPerro(ePerro list[], int length, int *idPerro,int cantidadIntentosPermitidos)
{
	...
	do
	{
		...
		pedirEntero(*idPerro, "Ingrese el Id del perro", "Reingrese Id invalido", 7000, 9999);
		intentos++;
	}while(Perro_findById(list, length, *idPerro)==-1&&intentos<cantidadIntentosPermitidos);
	if(Perro_findById(list, length, *idPerro)!=-1)
	{
		retorno=1;
	}
	return retorno;
